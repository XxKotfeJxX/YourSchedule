...
class CompanyCurriculumTab:
    ...
    def build(self) -> None:
        ...
        ttk.Label(header, text="Curriculum", style="CardTitle.TLabel").pack(anchor="w")
        ttk.Label(
            header,
            text="Teachers, subjects, plans, assignments and requirement sync.",
    ...
    def _build_teachers_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(form, text="Teacher name", style="Card.TLabel").grid(row=0, column=0, sticky="w")
    ...
    def _add_teacher(self) -> None:
        ...
        if not name:
            messagebox.showerror("Validation error", "Teacher name is required.")
            return
        try:
            with session_scope() as session:
            ...
            messagebox.showerror("Conflict", "Teacher with this name already exists.")
            return
        ...
        self._load_teachers()
        self._load_plan_reference_data()
        self._set_status(f"Teacher '{name}' created.")

    def _update_teacher(self) -> None:
        teacher_id = self._selected_listbox_id(self.teacher_listbox, self._teacher_ids)
        if teacher_id is None:
            messagebox.showerror("Validation error", "Select a teacher first.")
            return
        name = self.teacher_name_var.get().strip()
        if not name:
            messagebox.showerror("Validation error", "Teacher name is required.")
        ...
        self._load_teachers()
        self._load_plan_reference_data()
        self._set_status(f"Teacher #{teacher_id} updated.")
    ...
    def _delete_teacher(self) -> None:
        ...
        if teacher_id is None:
            messagebox.showerror("Validation error", "Select a teacher first.")
            return
        if not messagebox.askyesno("Delete teacher", "Delete selected teacher?"):
            return
        try:
            with session_scope() as session:
        ...
        self._load_teachers()
        self._load_plan_reference_data()
        self._set_status(f"Teacher #{teacher_id} deleted.")
    ...
    def _build_subjects_tab(self, parent: ttk.Frame) -> None:
        ...
        form.grid(row=0, column=1, sticky="nsew")
        form.grid_columnconfigure(0, weight=1)
        ttk.Label(form, text="Subject name", style="Card.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.subject_name_var).grid(row=1, column=0, sticky="ew", pady=(4, 8))
        ttk.Label(form, text="Code (optional)", style="Card.TLabel").grid(row=2, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.subject_code_var).grid(row=3, column=0, sticky="ew", pady=(4, 8))
        ttk.Label(form, text="Department (optional)", style="Card.TLabel").grid(row=4, column=0, sticky="w")
    ...
    def _load_subjects(self) -> None:
        ...
        for item in subjects:
            ...
            dept_name = self._department_name_by_id.get(item.department_id, "n/a")
    ...
    def _on_subject_select(self) -> None:
        ...
        if subject.department_id is None:
            ...
            self.subject_department_var.set(
                f"{subject.department_id} | {self._department_name_by_id.get(subject.department_id, f'Department #{subject.department_id}')}"
            )

    def _add_subject(self) -> None:
        name = self.subject_name_var.get().strip()
        if not name:
            messagebox.showerror("Validation error", "Subject name is required.")
            return
        try:
            with session_scope() as session:
            ...
            messagebox.showerror("Conflict", "Subject with this name or code already exists.")
            return
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
            return
        ...
        self._set_status(f"Subject '{name}' created.")

    def _update_subject(self) -> None:
        subject_id = self._selected_listbox_id(self.subject_listbox, self._subject_ids)
        if subject_id is None:
            messagebox.showerror("Validation error", "Select a subject first.")
            return
        name = self.subject_name_var.get().strip()
        if not name:
            messagebox.showerror("Validation error", "Subject name is required.")
        ...
        try:
            ...
            return
        self._refresh_subjects()
        self._set_status(f"Subject #{subject_id} updated.")
    ...
    def _delete_subject(self) -> None:
        ...
        if subject_id is None:
            messagebox.showerror("Validation error", "Select a subject first.")
            return
        if not messagebox.askyesno("Delete subject", "Delete selected subject?"):
            return
        try:
            with session_scope() as session:
        ...
        self.subject_department_var.set("")
        self._refresh_subjects()
        self._set_status(f"Subject #{subject_id} deleted.")
    ...
    def _build_plans_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(left, text="Plans", style="CardTitle.TLabel").grid(row=0, column=0, sticky="w")
    ...
    def _build_plan_editor(self, parent: ttk.Frame) -> None:
        plan_editor = ttk.LabelFrame(parent, text="Plan", padding=8)
        plan_editor.grid(row=0, column=0, sticky="ew", pady=(0, 8))
        plan_editor.grid_columnconfigure(1, weight=1)
        plan_editor.grid_columnconfigure(3, weight=1)

        ttk.Label(plan_editor, text="Name").grid(row=0, column=0, sticky="w")
        ttk.Entry(plan_editor, textvariable=self.plan_name_var).grid(row=0, column=1, sticky="ew", padx=(6, 12), pady=(0, 6))
        ttk.Label(plan_editor, text="Semester").grid(row=0, column=2, sticky="w")
        ttk.Entry(plan_editor, textvariable=self.plan_semester_var, width=10).grid(row=0, column=3, sticky="w", padx=(6, 0), pady=(0, 6))

        ttk.Label(plan_editor, text="Specialty").grid(row=1, column=0, sticky="w")
        self.plan_specialty_box = ttk.Combobox(plan_editor, textvariable=self.plan_specialty_var, state="readonly")
        self.plan_specialty_box.grid(row=1, column=1, sticky="ew", padx=(6, 12), pady=(0, 6))
        ttk.Label(plan_editor, text="Course").grid(row=1, column=2, sticky="w")
        self.plan_course_box = ttk.Combobox(plan_editor, textvariable=self.plan_course_var, state="readonly")
        self.plan_course_box.grid(row=1, column=3, sticky="ew", padx=(6, 0), pady=(0, 6))

        ttk.Label(plan_editor, text="Stream").grid(row=2, column=0, sticky="w")
        self.plan_stream_box = ttk.Combobox(plan_editor, textvariable=self.plan_stream_var, state="readonly")
        self.plan_stream_box.grid(row=2, column=1, sticky="ew", padx=(6, 12), pady=(0, 6))

        buttons = ttk.Frame(plan_editor, style="Card.TFrame")
        buttons.grid(row=3, column=0, columnspan=4, sticky="w", pady=(2, 0))
        self._motion_button(buttons, text="Create", command=self._create_plan, primary=True, width=100, height=34).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Update", command=self._update_plan, primary=False, width=100, height=34).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Delete", command=self._delete_plan, primary=False, width=100, height=34).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Sync Plan", command=self._sync_plan, primary=False, width=120, height=34).pack(side=tk.LEFT)

    def _build_components_editor(self, parent: ttk.Frame) -> None:
        components = ttk.LabelFrame(parent, text="Components", padding=8)
        components.grid(row=1, column=0, sticky="nsew", pady=(0, 8))
        components.grid_columnconfigure(0, weight=1)
        components.grid_rowconfigure(0, weight=1)
        ...
        self._motion_button(buttons, text="Add", command=self._create_component, primary=True, width=90, height=32).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Update", command=self._update_component, primary=False, width=90, height=32).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Delete", command=self._delete_component, primary=False, width=90, height=32).pack(side=tk.LEFT)

    def _build_assignments_editor(self, parent: ttk.Frame) -> None:
        assignments = ttk.LabelFrame(parent, text="Assignments", padding=8)
        assignments.grid(row=2, column=0, sticky="nsew")
        assignments.grid_columnconfigure(0, weight=1)
        assignments.grid_rowconfigure(0, weight=1)
        ...
        ttk.Label(form, text="Teacher").grid(row=0, column=0, sticky="w")
        ...
        ttk.Label(form, text="Target type").grid(row=0, column=2, sticky="w")
        ...
        ttk.Label(form, text="Stream").grid(row=1, column=0, sticky="w")
        ...
        ttk.Label(form, text="Group").grid(row=1, column=2, sticky="w")
        ...
        ttk.Label(form, text="Subgroup").grid(row=1, column=4, sticky="w")
        ...
        ttk.Label(form, text="Sessions (blank = component)").grid(row=2, column=0, sticky="w")
        ttk.Entry(form, textvariable=self.assignment_sessions_var, width=10).grid(row=2, column=1, sticky="w", padx=(6, 10), pady=(0, 4))
        ttk.Label(form, text="Max/W (blank = component)").grid(row=2, column=2, sticky="w")
        ...
        self._motion_button(buttons, text="Add", command=self._create_assignment, primary=True, width=90, height=32).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Update", command=self._update_assignment, primary=False, width=90, height=32).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Delete", command=self._delete_assignment, primary=False, width=90, height=32).pack(side=tk.LEFT, padx=(0, 6))
        self._motion_button(buttons, text="Sync", command=self._sync_assignment, primary=False, width=90, height=32).pack(side=tk.LEFT)
    ...
    def _load_plans(self) -> None:
        ...
        for item in plans:
            sem_suffix = f" | sem {item.semester}" if item.semester is not None else ""
    ...
    def _on_plan_select(self) -> None:
        ...
        self.plan_stream_var.set(
            "" if plan.stream_id is None else f"{plan.stream_id} | {self._stream_name_by_id.get(plan.stream_id, f'Stream #{plan.stream_id}')}"
        )
    ...
    def _create_plan(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Conflict", "Plan with this name already exists.")
            return
        except Exception as exc:
            messagebox.showerror("Error", str(exc))
            return
        self._load_plans()
        self._set_status(f"Plan '{name}' created.")

    def _update_plan(self) -> None:
        if self._selected_plan_id is None:
            messagebox.showerror("Validation error", "Select a plan first.")
            return
        name = self.plan_name_var.get().strip()
        if not name:
            messagebox.showerror("Validation error", "Plan name is required.")
        ...
        try:
            ...
            return
        self._load_plans()
        self._set_status(f"Plan #{self._selected_plan_id} updated.")

    def _delete_plan(self) -> None:
        if self._selected_plan_id is None:
            messagebox.showerror("Validation error", "Select a plan first.")
            return
        if not messagebox.askyesno("Delete plan", "Delete selected plan with components and assignments?"):
            return
        ...
        self._selected_assignment_id = None
        self._load_plans()
        self._set_status(f"Plan #{plan_id} deleted.")
    ...
    def _sync_plan(self) -> None:
        ...
        self._set_status(f"Synchronized {len(synced)} assignment(s) for plan #{self._selected_plan_id}.")
    ...
    def _load_components(self, plan_id: int | None) -> None:
        ...
        for item in components:
            iid = self.component_tree.insert(
                ...
                values=(
                    item.id,
                    self._subject_name_by_id.get(item.subject_id, f"Subject #{item.subject_id}"),
    ...
    def _on_component_select(self) -> None:
        ...
        self.component_subject_var.set(
            f"{component.subject_id} | {self._subject_name_by_id.get(component.subject_id, f'Subject #{component.subject_id}')}"
        )
    ...
    def _create_component(self) -> None:
        ...
        if subject_id is None:
            messagebox.showerror("Validation error", "Select a subject.")
            return
        try:
            with session_scope() as session:
            ...
            return
        self._load_components(self._selected_plan_id)
        self._set_status(f"Component added to plan #{self._selected_plan_id}.")

    def _update_component(self) -> None:
        if self._selected_component_id is None:
            messagebox.showerror("Validation error", "Select a component first.")
            return
        subject_id = self._parse_prefixed_id(self.component_subject_var.get())
        if subject_id is None:
            messagebox.showerror("Validation error", "Select a subject.")
        ...
        if self._selected_plan_id is not None:
            self._load_components(self._selected_plan_id)
        self._set_status(f"Component #{self._selected_component_id} updated.")

    def _delete_component(self) -> None:
        if self._selected_component_id is None:
            messagebox.showerror("Validation error", "Select a component first.")
            return
        if not messagebox.askyesno("Delete component", "Delete selected component and all assignments?"):
            return
        ...
        if self._selected_plan_id is not None:
            self._load_components(self._selected_plan_id)
        self._set_status(f"Component #{component_id} deleted.")
    ...
    def _load_assignments(self, component_id: int | None) -> None:
        ...
        for item in assignments:
            iid = self.assignment_tree.insert(
                ...
                values=(
                    item.id,
                    self._teacher_name_by_id.get(item.teacher_resource_id, f"Teacher #{item.teacher_resource_id}"),
    ...
    def _target_display(self, target_type: PlanTargetType, stream_id: int | None, target_resource_id: int | None) -> str:
        if target_type == PlanTargetType.STREAM:
            return f"STREAM: {self._stream_name_by_id.get(stream_id or 0, f'#{stream_id}')}"
        if target_type == PlanTargetType.GROUP:
            return f"GROUP: {self._group_name_by_id.get(target_resource_id or 0, f'#{target_resource_id}')}"
        return f"SUBGROUP: {self._subgroup_name_by_id.get(target_resource_id or 0, f'#{target_resource_id}')}"
    ...
    def _on_assignment_select(self) -> None:
        ...
        self.assignment_teacher_var.set(
            f"{assignment.teacher_resource_id} | {self._teacher_name_by_id.get(assignment.teacher_resource_id, f'Teacher #{assignment.teacher_resource_id}')}"
        )
        ...
        if assignment.stream_id is not None:
            self.assignment_stream_var.set(
                f"{assignment.stream_id} | {self._stream_name_by_id.get(assignment.stream_id, f'Stream #{assignment.stream_id}')}"
            )
        if assignment.target_type == PlanTargetType.GROUP and assignment.target_resource_id is not None:
            self.assignment_group_var.set(
                f"{assignment.target_resource_id} | {self._group_name_by_id.get(assignment.target_resource_id, f'Group #{assignment.target_resource_id}')}"
            )
        if assignment.target_type == PlanTargetType.SUBGROUP and assignment.target_resource_id is not None:
            self.assignment_subgroup_var.set(
                f"{assignment.target_resource_id} | {self._subgroup_name_by_id.get(assignment.target_resource_id, f'Subgroup #{assignment.target_resource_id}')}"
            )
    ...
    def _create_assignment(self) -> None:
        ...
        if teacher_id is None:
            messagebox.showerror("Validation error", "Select a teacher.")
            return
        try:
            ...
            return
        self._load_assignments(self._selected_component_id)
        self._set_status("Assignment added and synchronized.")

    def _update_assignment(self) -> None:
        if self._selected_assignment_id is None:
            messagebox.showerror("Validation error", "Select an assignment first.")
            return
        teacher_id = self._parse_prefixed_id(self.assignment_teacher_var.get())
        if teacher_id is None:
            messagebox.showerror("Validation error", "Select a teacher.")
        ...
        if self._selected_component_id is not None:
            self._load_assignments(self._selected_component_id)
        self._set_status("Assignment updated and synchronized.")

    def _delete_assignment(self) -> None:
        if self._selected_assignment_id is None:
            messagebox.showerror("Validation error", "Select an assignment first.")
            return
        if not messagebox.askyesno("Delete assignment", "Delete selected assignment?"):
            return
        ...
        if self._selected_component_id is not None:
            self._load_assignments(self._selected_component_id)
        self._set_status(f"Assignment #{assignment_id} deleted.")
    ...
    def _sync_assignment(self) -> None:
        ...
        if self._selected_component_id is not None:
            self._load_assignments(self._selected_component_id)
        self._set_status(f"Assignment #{assignment_id} synchronized.")
